...
-- Índice compuesto por orden dentro de la venta para consultas
CREATE INDEX idx_detalles_venta_venta_orden ON detalles_venta(id_venta, orden);

-- CORREGIDO: Agregar constraint UNIQUE sin IF NOT EXISTS
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Índice por orden dentro de la compra
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'ux_compra_producto_orden'
    ) THEN
        ALTER TABLE compra_producto
            ADD CONSTRAINT ux_compra_producto_orden UNIQUE (id_compra, orden);
    END IF;
END $$;
